...
import NbIpaV2Model from "../models/NaiveBayesV2Model.js";
...
export const createTrainingData = async (req, res) => {
  try {
    ...
    await NbIpaV2Model.destroy({ where: {} });
    ...
    res.status(200).json({
      ...
      data: await NbIpaV2Model.findAll()
    });
...
export const setMean = async (res) => {
  try {
    ...
    const dataset = await NbIpaV2Model.findAll();    
...
export const setStdev = async (res) => {
  try {
    ...
    const dataset = JSON.parse(JSON.stringify(await NbIpaV2Model.findAll()));
...
export const calcNormDist = async (x1, x2, x3, x4, x5, x6, x7, x8, x9, x10, x11, x12, x13, x14, x15) => {
  try {
    // Mendapatkan semua data dari model NbIpaV2Model
    const dataset = await NbIpaV2Model.findAll();
